...
// ==========================================
  Future<void> _gercekSkorlariYukle() async {
    ...
    for (var p in masadakiHerkes) {
      if (p != ben) {
        int rakipSkor =
            widget.rakip1KumulatifSkor; // Ana menüden gelen varsayılan değer
        ...
        if (widget.odaKodu != null && widget.odaKodu!.isNotEmpty) {
          ...
          var res =
              await db.query('botlar', where: 'bot_adi = ?', whereArgs: [p]);
          if (res.isNotEmpty) {
...
// ==========================================
  void _canliOdaDinle() {
    ...
    _odaSubscription = FirebaseFirestore.instance
      ...
      if (odaTur > _guncelMevcutTur) {
        ...
        setState(() {
          for (var p in masadakiHerkes) {
            ...
            genelKumulatifSkorlar[p] =
                (genelKumulatifSkorlar[p] ?? 0) + (tumTurPuanlari[p] ?? 0);
            tumTurPuanlari[p] = 0;
          }
        ...
        puanlarMap.forEach((kullanici, pMap) {
          ...
          if (masadakiHerkes.contains(kName)) {
            ...
            rawP.forEach((k, v) {
              ...
              tumKategoriPuanlari.putIfAbsent(kName, () => {})[int.parse(k)] =
                  val;
              toplam += val;
            });
      ...
      if (rakipBekleniyor &&
          cevaplarMap.length >= masadakiHerkes.length &&
          trToLowerCase(kurucu) == trToLowerCase(ben) &&
          !turBittiMi) {
        setState(() {
          rakipBekleniyor = false;
      ...
      for (var yasakli in yasakliKelimeler) {
        ...
        for (var kelime in girilenKelimeler) {
          if (kelime == kucukYasakli || kelime.startsWith(kucukYasakli)) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                content: Text("Uygunsuz kelime tespiti!"),
      ...
      for (var kat in kategoriler) {
        ...
        for (var p in masadakiHerkes) {
          ...
          if (dogruluklar[id]![p]! > 0) {
            ...
            if (cevaplar[id]![p]!.length == enUzunDogruKelime[id]! &&
                enUzunDogruKelime[id]! > 0) {
              puan += 2;
            }
          }
...
// ==========================================
  Future<void> _sonrakiTuraGec() async {
    ...
    if (_guncelMevcutTur < widget.toplamTurSayisi) {
      ...
      // ==========================================
      // 🚀 YENİ MOTOR ENTEGRASYONU: MAÇ BİTİŞİ (PUAN BUG'I ÇÖZÜLDÜ)
      // ==========================================

      // 1. Sadece o anki maç skorlarını topluyoruz, genel kumülatife BÖLÜM 21'de dokunmuyoruz.
      for (var p in masadakiHerkes) {
        macSkorlari[p] = (macSkorlari[p] ?? 0) + (tumTurPuanlari[p] ?? 0);
        tumTurPuanlari[p] = 0;
      }

      // 2. Benim bu maçtaki skorum ve 1.lik durumu
      ...
      // 3. SADECE BU MAÇTA KAZANILAN SAF PUAN (HATA BURADAYDI)
      int safKazanilanPuan = benimMacSkorum + (birinciMiyim ? 100 : 0);

      // 4. ESKİ PUAN VE SIRALAMAYI AL
      ...
      // 5. PUANI VERİTABANINA KAYDET VE BOTLARI GÜNCELLE
      // DİKKAT: saveOyuncuSkor üzerine ekleme (increment) yaptığı için ona SADECE kazanılan ham puanı veriyoruz.
      await DatabaseHelper.instance.saveOyuncuSkor(ben, safKazanilanPuan);

      // Botlar için de aynı şekilde toplam puanı değil, sadece o maç kazandıkları SAF puanı gönderiyoruz.
      if (widget.odaKodu == null || widget.odaKodu!.isEmpty) {
      ...
      // 6. YENİ PUAN VE YENİ SIRALAMAYI AL (Veritabanı güncellendikten sonra yeni halini çekiyoruz)
      ...
      // 7. BÜTÜN DOĞRU VERİLERİ RESULT SAYFASINA FIRLAT
      if (context.mounted) {
...
// ==========================================
// BÖLÜM 22: GÖRSEL TASARIM VE KULLANICI ARAYÜZÜ (UI) (Kompakt Tek Ekran Tasarımı)
// ==========================================
  @override
  Widget build(BuildContext context) {
    ...
    if (turBittiMi) {
      ...
      return Scaffold(
        ...
        body: SafeArea(
          child: Column(
            children: [
              // 1. DÜZELTME: Üst skor tabelası yüksekliği 90'dan 75'e düşürüldü
              Container(
                height: 75,
              ...
              Expanded(
                child: ListView.builder(
                  padding: const EdgeInsets.symmetric(
                      horizontal: 16, vertical: 4), // 8'den 4'e düşürüldü
                  ...
                  itemBuilder: (context, index) {
                    if (index == kategoriler.length) {
                      ...
                      // 2. DÜZELTME: Satır arası boşluklar (padding) 16'dan 6'ya düşürüldü
                      return Padding(
                    ...
                    // 🔴🔴 BURADA DEĞİŞİKLİK YAPILDI: Kendi yazdığınız trToUpperCase() kullanıldı 🔴🔴
                    String benimKelime =
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Column(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                // 3. DÜZELTME: İkon boyutu biraz küçültüldü
                                Container(
                                  width: 38,
                                  height: 38,
                                  decoration: BoxDecoration(
                                ...
                                const SizedBox(height: 2),
                                SizedBox(
                                  // 🔴🔴 BURADA DEĞİŞİKLİK YAPILDI: Kategori isimleri sığsın diye width 60'tan 75'e çıkarıldı 🔴🔴
                                  width: 75,
                                  child: Text(
                                    // 🔴🔴 BURADA DEĞİŞİKLİK YAPILDI: toUpperCase yerine trToUpperCase kullanıldı 🔴🔴
                                    trToUpperCase(
                                        kategoriler[index]["isim"].toString()),
                                    textAlign: TextAlign.center,
                                    maxLines: 1,
                                    overflow: TextOverflow.ellipsis,
                                    style: const TextStyle(
              ...
              Padding(
                ...
                child: Column(
                  children: [
                      ...
                      Padding(
                        ...
                        child: Text(
                          hazirOyuncuSayisi >= masadakiHerkes.length
                              ...
                              : "⏳ $hazirOyuncuSayisi / ${masadakiHerkes.length} Oyuncu Hazır",
                          style: const TextStyle(
                    ...
                    SizedBox(
                      width: double.infinity,
                      height: 45, // Buton yüksekliği 50'den 45'e düşürüldü
    ...
    return Scaffold(
      appBar: PreferredSize(
        ...
        child: AppBar(
            title: const Text("İsim Şehir Hayvan Oyunu",
        ...
        child: Column(
          children: [
            Row(
              ...
              children: [
                Text("Tur: $_guncelMevcutTur / ${widget.toplamTurSayisi}",
                    style: const TextStyle(
            ...
            Text("${mevcutKategori["isim"]} Kategorisi",
                style: const TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.bold,
                    color: Colors.purple)),
            ...
            if (erkenBitirmeBonusuKazandiMi)
              Container(
                ...
                child: Column(
                  children: [
                    const Text("⚡ 20 SANİYE KURALI BAŞLATILDI!",
                        style: TextStyle(
                            color: Colors.amber,
                            fontWeight: FontWeight.bold,
                            fontSize: 16)),
